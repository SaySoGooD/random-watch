...
class RandomAnyItemModel(BaseModel):
    ...
    item: MovieResponseModel | TvResponseModel = Field(
        description="The movie or TV show data"
    )
...
class RandomAnyResponseModel(BaseModel):
    ...
    items: list[RandomAnyItemModel] = Field(
        description="List of randomly selected movies and TV shows"
    )
