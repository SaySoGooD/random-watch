...
from random_watch.infrastructure.api.ratelimit_middleware import GlobalRateLimitMiddleware
...
def setup_middlewares(app: FastAPI, /, config: Config) -> None:
    limiter = Limiter(
        ...
        default_limits=[f"{config.API_RATE_LIMIT_PER_CLIENT}/{config.API_RATE_LIMIT_WINDOW}seconds"],
    )
